...
def load_project_artifacts(project_dir: Path, variant: str = "safe") -> ProjectArtifacts:
    ...
    return ProjectArtifacts(
        ...
        # --title-suffix 와 story.md 선언이 겹치면 'U U' 가 된다. 이미 붙어 있으면 다시 붙이지 않는다.
        title=title if TITLE_SUFFIX and title.endswith(TITLE_SUFFIX.strip()) else title + TITLE_SUFFIX,
        prologue=prologue,
...
def run_sync(
    ...
    is_existing_project = target_url != DEFAULT_LOGIN_URL and any(k in target_url for k in ("projects/", "builder/", "storyId="))
    print("=" * 75)
...
def main() -> int:
    ...
    sync_parser.add_argument("--title-suffix", default="",
                             help="제목 뒤에 붙일 문자열. story.md Core 에 '- Unsafe title suffix:' 가 있으면 unsafe 판에 자동으로 붙으므로 쓰지 않는다")
