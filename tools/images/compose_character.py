...
def _search_paths() -> list[Path]:
    """Danbooru wiki DB lookup order.

    CRACK_WIKI_DB wins so a user can point at any copy without editing source.
    Everything after it is a convention, not a guarantee — the DB ships with
    novel-ai-image-skill, not with this repository.
    """
    paths: list[Path] = []
    env = os.environ.get("CRACK_WIKI_DB")
    if env:
        paths.append(Path(env).expanduser())
    here = Path(__file__).resolve().parent
    paths.append(here / "data" / "wiki.sqlite3")
    paths.append(here.parents[1] / "data" / "wiki.sqlite3")
    rel = Path("novel-ai-image-skill") / "skills" / "novel-ai-image-skill" / "data" / "wiki.sqlite3"
    paths.append(here.parents[2] / rel)
    paths.append(Path.home() / "crack" / rel)
    paths.append(Path.home() / rel)
    return paths
